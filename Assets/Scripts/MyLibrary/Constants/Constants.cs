    ...
    public class Constants {
        ...
        private static void CreateConstantFromEntry( ConstantsEntry i_entry ) {
            switch ( i_entry.Type ) {
                ...
                case STRINGLIST_KEY:
                    ...
                    break;
                default:
                    mMessenger.Send<LogTypes, string, string>( MyLogger.LOG_EVENT, LogTypes.Error, "Illegal constant type " + i_entry.Type + " for key " + i_entry.ID, "" );
                    break;
            }
        }
        ...
        public static T GetConstant<T>( string strKey, T defaultVal ) {
            if ( mConstants.Count == 0 ) {
                mMessenger.Send<LogTypes, string, string>( MyLogger.LOG_EVENT, LogTypes.Fatal, "Constants being accessed before init!", "" );
            }
            ...
            if ( mConstants.ContainsKey( strKey ) ) {
                ...
                mMessenger.Send<LogTypes, string, string>( MyLogger.LOG_EVENT, LogTypes.Fatal, "No such constant for key " + strKey, "" );
            }
        ...
        public static Vector3 ParseVector3( string i_str ) {
            ...
            if ( arrayVector3.Length == 3 ) {
                ...
                mMessenger.Send<LogTypes, string, string>( MyLogger.LOG_EVENT, LogTypes.Fatal, "Illegal vector3 parsing for " + i_str + " -- reverting to 0,0,0", "" );
            }
        ...
        private static Color ParseColor( string i_str ) {
            ...
            if ( arrayColor.Length == 4 ) {
                ...
                mMessenger.Send<LogTypes, string, string>( MyLogger.LOG_EVENT, LogTypes.Fatal, "Illegal color parsing for " + i_str + " -- reverting to white", "" );
            }
